...
const usePersistedReducer = (reducer, initialState, localStorageKey) => {  // the reducer function is from line 17
    const [state, dispatch] = useReducer(reducer, initialState, initial => {   // initial here represent the initializer
    ...
    useEffect(() => {     // useEffect will set the value of localStorage every time the state changes.
...
const starredShowsReducer = (currentStarred, action) => {  // this is the reducer function after changing the state is return the value to
    switch (action.type) {                                              // usePersistedReducer which will return new persisted value
        case 'STAR':
